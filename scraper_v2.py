"""
Incremental Art University Job Scraper.

Efficiently scrapes German-speaking art universities for:
- Wissenschaftliche Mitarbeiter positions
- Künstlerische Mitarbeiter positions

Uses content hashing to detect changes and only scrapes when content has changed.
"""
...
class IncrementalScraper:
    """Main scraper class with incremental update capability."""

    def __init__(self, db_path='art_positions.db', use_selenium=False, delay=2):
        """
        Initialize scraper.

        Args:
            db_path: Path to SQLite database
            use_selenium: Use Selenium for all requests (slower but works with JS)
            delay: Delay in seconds between requests
        """
        self.db = DatabaseManager(db_path)
        self.fetcher = PageFetcher(use_selenium=use_selenium)
    ...
    def _log_success(self, university_name: str, positions_found: int, timestamp: str):
        ...
        log = ScrapingLog(
            university_name=university_name,
            status='success',
            message=f'Found {positions_found} new positions',
            timestamp=timestamp,
            positions_found=positions_found,
        )
        self.db.add_log(log)
